...
export const updateInvoiceSchema = z.object({
    student_id: z.union([z.string().min(1, 'student_id, amount, and due_date are required.'), z.number()]).optional(),
    amount: z.union([z.string(), z.number()]).refine(val => {
        const num = Number(val);
        return !isNaN(num) && num > 0;
    }, 'Amount must be a positive number.').optional(),
    due_date: z.string().min(1, 'student_id, amount, and due_date are required.').optional(),
    dueDate: z.string().optional(),
    fee_structure_id: z.union([z.string(), z.number()]).optional(),
    feeStructureId: z.union([z.string(), z.number()]).optional(),
    academic_year: z.string().optional(),
